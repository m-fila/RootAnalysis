...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
ELIAnalyzer::ELIAnalyzer(const std::string & aName, const std::string & aDecayMode) : Analyzer(aName){
...
#pragma omp critical
        {
                ...
                //std::string dataPUFileName = "ELIp://akalinow.web.cern.ch/akalinow/Data_Pileup_2016_271036-284044_13TeVMoriond17_23Sep2016ReReco_69p2mbMinBiasXS.root";
		std::string dataPUFileName = "ELIp://akalinow.web.cern.ch/akalinow/Data-MC_Pileup_B-C-12Sep_D-E-Prompt_Cert_294927-305185_13TeV_PromptReco_Collisions17_69mb.root";

                puDataFile_ = TFile::Open(dataPUFileName.c_str(),"CACHEREAD");

                //std::string mcPUFileName = "ELIp://akalinow.web.cern.ch/akalinow/MC_Spring16_PU25ns_V1.root";
                //std::string mcPUFileName = "ELIp://akalinow.web.cern.ch/akalinow/MC_Moriond17_PU25ns_V1.root";
		std::string mcPUFileName = "ELIp://akalinow.web.cern.ch/akalinow/MC_Pileup_2017MCv1_800bins.root";
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
Analyzer* ELIAnalyzer::clone() const {

        std::string myDecayMode = myChannelSpecifics->getDecayModeName();
        ELIAnalyzer* clone = new ELIAnalyzer(name(),myDecayMode);
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void ELIAnalyzer::initialize(TDirectory* aDir,
                             pat::strbitset *aSelections){

        mySelections_ = aSelections;

        myHistos_ = new ELIHistograms(aDir, selectionFlavours_, myChannelSpecifics->getDecayModeName());
}
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void ELIAnalyzer::finalize(){

        myHistos_->finalizeHistograms(myChannelSpecifics->getCategoryRejester());
}
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void ELIAnalyzer::setAnalysisObjects(const EventProxyELI & myEventProxy){
        ...
        aSeparatedJets = ELIAnalysis::getSeparatedJets(myEventProxy, aLeg1, aLeg2, 0.5);
        aJet1 = aSeparatedJets.size() ? aSeparatedJets[0] : ELIParticle();
        aJet2 = aSeparatedJets.size()>1 ? aSeparatedJets[1] : ELIParticle();
	aBJet1 = ELIParticle();
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
float ELIAnalyzer::getSystWeight(const ELIAnalysis::sysEffects & aSystEffect){

        if(aSystEffect==ELIAnalysis::NOMINAL) return 1.0;

        if(aSystEffect==ELIAnalysis::ZPtUp && sampleName.find("DYJets")!=std::string::npos) return aEvent.getPtReWeight();
        else if(aSystEffect==ELIAnalysis::ZPtDown && sampleName.find("DYJets")!=std::string::npos) return 1.0/aEvent.getPtReWeight();
        else if(aSystEffect==ELIAnalysis::TTUp && sampleName.find("TTbar")!=std::string::npos) return aEvent.getPtReWeight();
        else if(aSystEffect==ELIAnalysis::TTDown && sampleName.find("TTbar")!=std::string::npos) return 1.0/aEvent.getPtReWeight();
        else if((aSystEffect==ELIAnalysis::J2TUp || aSystEffect==ELIAnalysis::J2TDown)
                && aLeg2.getProperty(PropertyEnum::mc_match)==6) {
                float delta = 0.2*aLeg2.getP4().Perp()/100.0;
                if(aLeg2.getP4().Perp()>200) delta = 0.4;
                if(aSystEffect==ELIAnalysis::J2TDown) delta*=-1;
                return 1-delta;
        }
        else return 1.0;
}
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void ELIAnalyzer::getPreselectionEff(const EventProxyELI & myEventProxy){
        ...
        if(true || ntupleFile_!=myEventProxy.getTTree()->GetCurrentFile()) {
                ...
                float genWeight = ELIAnalysis::getGenWeight(myEventProxy);
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
float ELIAnalyzer::getPUWeight(const EventProxyELI & myEventProxy){
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void ELIAnalyzer::fillControlHistos(const std::string & hNameSuffix, float eventWeight,
                                    const ELIAnalysis::sysEffects & aSystEffect){
        ...
        if(aSystEffect!=ELIAnalysis::NOMINAL) return;
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
bool ELIAnalyzer::fillVertices(const std::string & sysType, float eventWeight){
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
bool ELIAnalyzer::passCategory(unsigned int iCategory){
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
bool ELIAnalyzer::analyze(const EventProxyBase& iEvent, ObjectMessenger *aMessenger){

        bool runSystematics = false;

        const EventProxyELI & myEventProxy = static_cast<const EventProxyELI&>(iEvent);
        sampleName = ELIAnalysis::getSampleName(myEventProxy);
	...
	getPreselectionEff(myEventProxy);
        ...
        const std::vector<const ELIAnalysis::eventCategory*> & aCategoryRejester = myChannelSpecifics->getCategoryRejester();
        for(unsigned int iSystEffect = (unsigned int)ELIAnalysis::NOMINAL;
            iSystEffect<=(unsigned int)ELIAnalysis::ZmumuDown; ++iSystEffect) {

              if(!runSystematics && iSystEffect!=(unsigned int)ELIAnalysis::NOMINAL) continue;
              if(iSystEffect==(unsigned int)ELIAnalysis::DUMMY_SYS) continue;

                ELIAnalysis::sysEffects aSystEffect = static_cast<ELIAnalysis::sysEffects>(iSystEffect);
			   ...
			   sampleName.find("MatchT")!=std::string::npos) reweightDY = myChannelSpecifics->getDYReweight(categorySuffix, aSystEffect);
                        ...
                        systEffectName = ELIAnalysis::systEffectName(iCategory, iSystEffect, aCategoryRejester);
                ...
                if(aMessenger and std::string("MLObjectMessenger").compare((aMessenger->name()).substr(0,17))==0 ) // if NULL it will do nothing
                {
                    ...
                    try
                    {
                        ...
                        mess->putObjectVector(const_cast <const ELIParticle*>(&aJet1), "jets");
                        mess->putObjectVector(const_cast <const ELIParticle*>(&aJet2), "jets");
                        mess->putObjectVector(const_cast <const ELIParticle*>(&aLeg1), "legs");
                        mess->putObjectVector(const_cast <const ELIParticle*>(&aLeg2), "legs");
                        mess->putObject(const_cast <const ELIParticle*>(&aMET), "amet");
                        mess->putObject(const_cast <const ELIAnalysis::sysEffects*>(&aSystEffect), "systEffect");
                    ...
                    {
                        std::throw_with_nested(std::runtime_error("[ERROR] UNKNOWN ERROR IN ELIAnalyzer::analyze WHEN PUTTING DATA TO MLObjectMessenger!"));
                    }                    
                }		
        }
        return true;
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
bool ELIAnalyzer::analyze(const EventProxyBase& iEvent){
